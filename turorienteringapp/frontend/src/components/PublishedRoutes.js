...
const PublishedRoutes = () => {
  ...
  const fetchRoutes = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:8000/api/v1/tourRoutes/getAllPublishedTourRoutes",
        {
      ...
      setError(
        "There was a problem fetching the published routes. Please try again."
      );
    }
  };
    ...
    map.on("load", () => {
      routes.forEach((route, index) => {
        ...
        const popupContent = ReactDOMServer.renderToString(
          <RoutePopup route={route} distance={distance.toFixed(2)} />
        );
  ...
  return (
    <div className="relativeContainer">
      ...
      <button onClick={handleBackClick} className="PublishedRoutesBackButton">
        Back
      </button>
      <div className="mapContainer" ref={mapContainerRef}>
        <div className="PublishedRoutes-logo" onClick={handleLogoClick}>
          TurRuter
        </div>
      </div>
    </div>
