...
const MyRoutes = () => {
  ...
  const fetchRoutes = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8000/api/v1/tourRoutes/getIndividualUsersTourRoutes",
        {
          headers: { Authorization: `Bearer ${userToken}` },
        }
      );
  ...
  return (
    <div className="relativeContainer">
      ...
      <div className="MyRoutes-logo" onClick={handleLogoClick}>
        TurRuter
      </div>
      <button onClick={handleBackClick} className="MyRoutesBackButton">
        Back
      </button>
    </div>
  );
