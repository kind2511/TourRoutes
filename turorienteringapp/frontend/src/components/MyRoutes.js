...
const MyRoutes = () => {
    ...
    const calculateRouteDistance = (coordinates) => {
        ...
        for (let i = 0; i < coordinates.length - 1; i++) {
            const from = point(coordinates[i]);  // Convert current coordinate to point
            const to = point(coordinates[i + 1]);  // Convert next coordinate to point
            const options = { units: 'kilometers' };  // Define distance measurement unit
            totalDistance += distance(from, to, options);  // Calculate and accumulate distance
        }
    
        return totalDistance.toFixed(2);  // Return total distance with 2 decimal places
    };
            ...
            routes.forEach((route, index) => {
                if (route.coordinates && route.coordinates.length > 0) {
                    ...
                    route.coordinates.forEach((coord, coordIndex) => {
                        ...
                        // Add the route name and distance for the first and last point of each route
                        if(coordIndex === 0 || coordIndex === route.coordinates.length - 1) {
                            ...
                            .setLngLat(coord)
                            .setHTML(`${route.name} - ${calculateRouteDistance(route.coordinates)} km`)
                            .addTo(map);
                        }
                    });
                }
