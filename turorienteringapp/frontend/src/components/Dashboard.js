...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const popupReviewInterval = setInterval(() => {
      setActivePopupReviewIndex(
        (prevIndex) => (prevIndex + 1) % popupReviews.length
      );
      setShowSlide(true);
  ...
  return (
    <>
      <div className="navigation-options">
        ...
        <span className="option-item" onClick={handleProfile}>
          My Profile
        </span>
        <div className="edit-dropdown">
          <span className="option-item">Routes</span>
          <div className="edit-dropdown-content">
            <a href="#" onClick={() => navigate("/new-route")}>
              New Route
            </a>
            <a href="#" onClick={() => navigate("/my-routes")}>
              My Routes
            </a>
            <a href="#" onClick={() => navigate("/published-routes")}>
              Published Routes
            </a>
            <a href="#" onClick={handleFindPath}>
              Find Path
            </a>
          </div>
        </div>
        <span className="option-item" onClick={handleLogout}>
          Logout
        </span>
        {/* Conditional rendering of Admin link based on userRole */}
        {userRole === "admin" && (
          <span className="option-item admin-button" onClick={handleAdmin}>
            Admin
          </span>
        )}
      </div>
      ...
      <div className="review-panel">{reviews[activeReviewIndex]}</div>
    </>
  );
};
